...
shared interface Editor satisfies ImmediateNarrowingTransformer<Node> {
    ...
    shared actual default ModuleImport transformModuleImport(ModuleImport that)
            => editNode(that) then that.copy(transformFullPackageNameOrStringLiteral(that.name), transformStringLiteral(that.version), transformAnnotations(that.annotations), nullsafeInvoke(that.repository, transformLIdentifier)) else that;
