...
shared class CeylonExpressionTransformer(String indentLevel = "    ") satisfies ImmediateNarrowingTransformer<String> {
    ...
    transformAssertion(Assertion that)
            => that.message exists
                    then "Assertion {
                          ...
                          `` indent + indentLevel ``message = ``transformWithIndent(that.message)``;
                          ``indent``}"
                    else "Assertion(``transformWithIndent(that.conditions)``)";
    ...
    transformDefaultedValueParameter(DefaultedValueParameter that)
            => "DefaultedValueParameter {
                ...
                `` indent + indentLevel ``specifier = ``transformWithIndent(that.specifier)``;
                ``indent``}";
    transformDestructure(Destructure that) => "Destructure(``transformWithIndent(that.patterns)``)";
    ...
    transformModuleBody(ModuleBody that)
            => that.constantDefinitions.empty
                    then "ModuleBody(``transformWithIndent(that.moduleImports)``)"
                    else "ModuleBody {
                          `` indent + indentLevel ``moduleImports = ``transformWithIndent(that.moduleImports)``;
                          `` indent + indentLevel ``constantDefinitions = ``transformWithIndent(that.constantDefinitions)``;
                          ``indent``}";
    ...
    transformModuleSpecifier(ModuleSpecifier that)
            => "ModuleSpecifier {
                ...
                `` indent + indentLevel ``classifier = ``transformWithIndent(that.classifier)``;
                ``indent``}";
