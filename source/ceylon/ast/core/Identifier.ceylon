...
shared abstract class Identifier(name, usePrefix) of LIdentifier | UIdentifier extends Node() {
    ...
    "Whether this identifier is a valid Ceylon identifier.
     The [[LIdentifier.internal]] and [[UIdentifier.internal]] constructors
     allow compilers and other tools to create identifiers that contain dollar signs;
     these identifiers will never collide with regular Ceylon identifiers,
     because they are not valid in Ceylon."
    shared Boolean valid => !name.contains('$');
...
shared class LIdentifier extends Identifier {
    
    shared new (String name, Boolean usePrefix = false) extends Identifier(name, usePrefix) {
        "Name must not be empty"
    ...
    "An initial lowercase identifier generated by the compiler (typechecker or backend).
     These identifiers may also contain dollar signs ('$'), to avoid collisions with regular identifiers."
    shared new internal(String name, Boolean usePrefix = false) extends Identifier(name, usePrefix) {
        "Name must not be empty"
        assert (exists first = name.first);
...
shared class UIdentifier extends Identifier {
    ...
    "An initial uppercase identifier generated by the compiler (typechecker or backend).
     These identifiers may also contain dollar signs ('$'), to avoid collisions with regular identifiers."
    shared new internal(String name, Boolean usePrefix = false) extends Identifier(name, usePrefix) {
        "Name must not be empty"
        assert (exists first = name.first);
